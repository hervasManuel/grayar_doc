...
A la hora de calcular la geometría 3D del escenario visto a través de las cámaras son las \textit{homografías planares} o, simplemente, \textit{homografías} \cite{Martinez03}.
...
\begin{figure}[tb]
  \begin{minipage}{13.1cm}
    ...
    \ifthenelse{\equal{Transformación de un punto P del mundo real utilizando una homografía. Fuente
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.6\textwidth]{parametrosCamara.pdf}
...
\subsection{Modelado Cámara-Proyector mediante visión estéreo}
...
\end{itemize}
  ...
  (imagen 1, ejes de coordenadas y plano)
...
Son parámetros que identifican de forma exclusiva la transformación entre el marco de referencia desconocido de la camra y el marco de referencia mundialmenteconocido.
...
%\begin{equation}
%x=-\left(x_{im}-0{x}\right)s_x ó x_{im}=-{\frac{x}{s_x}+0_x}

%y=-\left(y_{im}-0{y}\right)s_y ó y_{im}=-{\frac{y}{s_y}+0_y}
%\end{equation}
...
\end{equation}
\subsubsection{Relacionar coordenadas píxel en coordenadas reales}
...
Vamos a verificar la corrección de la matriz anterior:

%\begin{equation}
%p=Mr_1^T_pPw=\begin{pmatrix}-fR_1^T & fR_1^TT \\-fR_2^T & fR_2^TT \\R_3^T & -R_3^TT\end{pmatrix}\begin{pmatrix}PW\\1\end{pmatrix}=\begin{pmatrix}-fR_1^T\left(P_w-T\right)\\ -fR_2^T\left(P_w-T\right)\\R_3^T\left(P_w-T\right)\end{pmatrix}
%\end{equation}
...
\subsection{El modelo de cámara afín}
\begin{itemize}
...
%Aquí la presencia de w se explica por el uso de homografía sistema de coordenadas (y w = Z ). Los parámetros desconocidos son f_x y f_y (Cámara longitudes focales) y (C_x, c_y) que son los centros ópticos expresadas en píxeles coordenadas. Si para ambos ejes una longitud focal común se utiliza con un dado un relación de aspecto (por lo general 1), entonces f_y = f_x * a y en la fórmula superior tendremos una sola longitud focal F . La matriz que contiene estos cuatro parámetros se conoce como la matriz de cámara. Mientras que los coeficientes de distorsión son los mismos independientemente de las resoluciones de cámara utilizados, éstos deben ampliarse junto con la resolución actual de la resolución calibrada.
